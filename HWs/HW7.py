...
_SOURCE_WEIGHTS = {
    # tweak if your CSV has these; unknowns default to 1.0
    "reuters": 1.15,
...
def _recency_score(published: datetime | None, now: datetime) -> float:
    if not published:
        return 0.4  # unknown date gets modest baseline
    days = (now - published).days
    if days < 0:  # future-dated? treat as fresh
        return 1.0
    # simple decay: <=1 day 1.0, 7 days ~0.6, 30 days ~0.2
    if days <= 1:
        return 1.0
    if days <= 7:
        return 0.6
...
def compute_interest_scores(df: pd.DataFrame) -> pd.Series:
    """
    Returns a score in [0, ~1.3] combining:
      50% recency, 35% keyword density, 15% source weight
    """
    ...
    def _row_score(row) -> float:
        ...
        ss = (_source_score(row.get("source")) - 1.0) * 0.15 + 0.15  # normalize
        return rs + ks + ss
...
def _hash_df(df: pd.DataFrame) -> str:
    """Hash rows relevant to embeddings to detect changes and persist under a stable key."""
    sig = io.BytesIO()
    # Only columns we embed
...
def _ensure_news_collection(df: pd.DataFrame, persist_dir: str, openai_key: str):
    ...
    # pick the best available text field for embeddings
...
def _prompt_rerank_for_interest(context_items: List[Dict[str, Any]]) -> str:
    """
    Ask the LLM to re-rank by 'interestingness' for a law firm client.
    """
    lines = []
...
def render():
    ...
    # CSV upload (or detect in news_data/)
    st.subheader("📄 Upload News CSV")
    up = st.file_uploader("Choose a CSV with columns like: title, summary/description/content, source, date, url", type=["csv"])
    ...
    if up:
        ...
        st.info("Using default CSV at ./news_data/news_dataset.csv")
        df = pd.read_csv(default_path)

    if df is None or df.empty:
        st.warning("Please upload a CSV or place one at ./news_data/news_dataset.csv")
        return

    # normalize expected columns
    # title (required-ish), summary or description or content (at least one), source, date/published, url
    cols_lower = {c.lower(): c for c in df.columns}
    rename_map = {}
    if "title" not in cols_lower:
        st.error("CSV must contain a 'title' column.")
        return
    # best-effort mapping for summary text
    text_col = None
    for cand in ["summary", "description", "content"]:
    ...
    if "date" not in cols_lower and "published" not in cols_lower and "pub_date" not in cols_lower and "time" not in cols_lower:
        df["date"] = ""
